...
1. Search for existing tasks first. If a matching task already exists, tell the user instead
   ...
   ```bash
   ~/.claude/skills/mavka/.claude/bin/mavka search tasks --query "..."
   ```
2. Create the task:
   ```bash
...
2. Update the task:
   ```bash
   ~/.claude/skills/mavka/.claude/bin/mavka task update <id> --status wip
   ~/.claude/skills/mavka/.claude/bin/mavka task update <id> --status done --tag django --tag completed
   ```
